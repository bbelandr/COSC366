...
router.get('/get_login', asyncMiddleware(async (req, res, next) => {
  const db = await dbPromise;
  const query = `SELECT * FROM Users WHERE username == ? AND password == ?;`;   // using placeholders to prevent SQL injection
  console.log(query)
  const result = await db.get(query, [req.query.username, req.query.password]); // Passing the values for the placeholders through here to avoid data mixing
